...
implementation{
    task void processCommand(){
        if(! call Queue.empty()){
			...
			signal CommandHandler.setTestClient(buff[0], buff[1], buff[2], buff[3], buff[4]);
                break;
            ...
            //// PROJECT 4 // TODO: FIX THIS
            case CMD_ROOM_HOST:
                dbg(COMMAND_CHANNEL, "Command Type: Room: Hosting\n");
                signal CommandHandler.setAppServer();
                break;

            case CMD_ROOM_CLI:
                dbg(COMMAND_CHANNEL, "Command Type: Room: Client\n");
                signal CommandHandler.setAppClient(buff);
                break;
